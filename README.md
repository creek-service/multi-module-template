...
* Multi-module Gradle Java project, including:
  ...
  * Code coverage tracking by [Codecov][6]
...
2. Customise the repo in GitHub `Settings`->:
   1. `General`->
      1. `Pull Requests`: 
...
4. Finish customising the repo in GitHub `Settings`->`Branches` and protect the `main` branch:
    ...
    3. Tick `Require status checks to pass before merging`
       ...
       2. With status checks:
          ...
          * `codecov/patch`
    4. Click `Create`.
5. Use the "Set next version" GitHub workflow to set the next version inline with other Creek repos.
...
* `./gradlew jacocoTestReport` will generate [Jacoco][5] coverage reports for each module.
